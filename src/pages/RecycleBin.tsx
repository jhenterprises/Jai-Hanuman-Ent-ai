    ...
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">Soft deleted applications and data managed here. Only Administrators can access this.</p>
        </div>
      </header>

      <div className="bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/50 rounded-[2.5rem] p-8 space-y-8 shadow-xl">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-4 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 transition-all shadow-inner"
            />
          </div>
          <div className="flex gap-4">
            <div className="relative">
              ...
              <select 
                ...
                className="pl-12 pr-10 py-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 rounded-2xl text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 transition-all appearance-none"
              >
                <option className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white" value="all">All Items</option>
                <option className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white" value="application">Applications Only</option>
                <option className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white" value="service">Services Only</option>
                <option className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white" value="user">Users Only</option>
                <option className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white" value="ledger">Ledger Entries Only</option>
              </select>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 space-y-6">
            <div className="w-24 h-24 bg-slate-100 dark:bg-slate-800/50 rounded-[2rem] flex items-center justify-center mx-auto text-slate-400 dark:text-slate-600 border border-slate-200 dark:border-white/5 shadow-2xl">
              <Database size={48} />
            </div>
            <div className="space-y-2">
              <h3 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">The bin is empty</h3>
          ...
          <div className="overflow-x-auto rounded-3xl border border-slate-200 dark:border-white/5 overflow-hidden">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-white/5 text-slate-500 text-[10px] uppercase tracking-[0.2em] font-black border-b border-slate-200 dark:border-white/5">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-white/5">
                {filteredItems.map((item, index) => (
                  <tr key={`${item.id}`} className="group hover:bg-slate-50 dark:hover:bg-white/10 transition-all duration-300">
                    <td className="py-6 px-6 text-slate-500 font-mono text-xs">{index + 1}</td>
                    <td className="py-6 px-6">
                      <div className="font-bold text-slate-800 dark:text-white font-mono tracking-tighter text-sm bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-white/5 inline-block">
                        {item.reference_number || item.id.substring(0, 8)}
                      </div>
                    </td>
                    <td className="py-6 px-6">
                      <div className="font-bold text-slate-800 dark:text-slate-200">{item.name || item.user_name}</div>
                      <div className="text-[10px] text-slate-500 dark:text-slate-400 font-medium uppercase tracking-widest">{item.type}</div>
                    </td>
                    <td className="py-6 px-6">
                    ...
                    <td className="py-6 px-6 text-slate-600 dark:text-slate-400 text-xs">
