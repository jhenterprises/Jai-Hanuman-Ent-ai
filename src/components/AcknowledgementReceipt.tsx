...
const AcknowledgementReceipt: React.FC<AcknowledgementReceiptProps> = ({ application, id }) => {
  ...
  return (
    <div id={id} className="bg-white text-slate-900 p-0 sm:p-8 max-w-4xl mx-auto shadow-2xl border border-slate-200 rounded-[2rem] overflow-hidden">
      {/* Header Section */}
      <div className="border-b-4 border-blue-600 p-8 bg-slate-50 flex flex-col items-center text-center space-y-4">
        ...
        <div>
          ...
          <p className="text-blue-600 font-bold tracking-[0.2em] text-sm uppercase mt-1">Application Acknowledgement</p>
        </div>
      </div>
      ...
      <div className="p-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-blue-50 border border-blue-100 p-4 rounded-2xl flex flex-col justify-center">
            <span className="text-[10px] uppercase tracking-widest text-blue-400 font-bold">Service Type</span>
            <span className="text-lg font-black text-slate-900 capitalize">{(application.service_name || application.service_type || '').replace(/-/g, ' ')}</span>
          </div>
          <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl flex flex-col justify-center">
            <span className="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Submission Date</span>
            <span className="text-sm font-bold text-slate-900">{submissionDate}</span>
          </div>
        </div>
        ...
        <div className="pt-8 border-t border-slate-200 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="text-center md:text-left space-y-1">
            <p className="text-sm font-black text-slate-900">JH Digital Seva Kendra</p>
            <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Authorized Service Center</p>
...
const DetailRow = ({ label, value, highlight = false, color = "" }: { label: string, value: string, highlight?: boolean, color?: string }) => (
  <div className="flex flex-col">
    <span className="text-[9px] uppercase tracking-widest text-slate-400 font-bold mb-0.5">{label}</span>
    <span className={`text-sm font-bold ${highlight ? (color || 'text-blue-600') : 'text-slate-900'} ${highlight ? 'uppercase tracking-wide' : ''}`}>
      {value || 'N/A'}
    </span>
  </div>
);
